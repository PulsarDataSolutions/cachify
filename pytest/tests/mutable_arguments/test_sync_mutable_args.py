...
class TestDictArguments:
    """Tests for caching behavior with dict arguments."""

    @pytest.fixture(autouse=True)
    def clear_cache(self):
        MemoryStorage.clear()

    def test_same_dict_returns_cached_result(self):
        call_count = 0
    ...
    def test_dict_with_different_insertion_order_produces_different_cache(self):
        """
        ...
        result1 = cached_func(dict1)
        result2 = cached_func(dict2)

        # Different insertion order = different cache keys (pickle preserves order)
        assert result1 != result2
        assert call_count == 2

    def test_different_dict_values_produce_different_cache(self):
        call_count = 0
        ...
        result1 = cached_func({"a": 1})
        result2 = cached_func({"a": 2})

        assert result1 != result2
        assert call_count == 2

    def test_nested_dict_same_order_caching(self):
        ...
        result1 = cached_func(nested1)
        result2 = cached_func(nested2)

        assert result1 == result2
        assert call_count == 1

    def test_nested_dict_different_order_produces_different_cache(self):
        """
...
class TestListArguments:
    """Tests for caching behavior with list arguments."""

    @pytest.fixture(autouse=True)
    def clear_cache(self):
        MemoryStorage.clear()

    def test_same_list_returns_cached_result(self):
        call_count = 0
    ...
    def test_list_order_matters(self):
        ...
        result1 = cached_func([1, 2, 3])
        result2 = cached_func([3, 2, 1])

        # List order matters, so these should be different cache entries
        assert result1 != result2
        assert call_count == 2

    def test_different_list_produces_different_cache(self):
        call_count = 0
    ...
    def test_nested_list_caching(self):
        call_count = 0
    ...
    def test_list_with_mixed_types(self):
        ...
        result1 = cached_func(mixed1)
        result2 = cached_func(mixed2)
...
class TestSetArguments:
    """Tests for caching behavior with set arguments."""

    @pytest.fixture(autouse=True)
    def clear_cache(self):
        MemoryStorage.clear()

    def test_same_set_returns_cached_result(self):
        call_count = 0

        @cache(ttl=TTL)
        def cached_func(items: set) -> int:
            nonlocal call_count
            call_count += 1
            return call_count
    ...
    def test_set_order_does_not_matter(self):
        ...
        result1 = cached_func(set1)
        result2 = cached_func(set2)

        # Sets are unordered, so these should hit the same cache
        assert result1 == result2
        assert call_count == 1

    def test_different_set_produces_different_cache(self):
        call_count = 0
    ...
    def test_frozenset_caching(self):
        """Frozensets should also work as cache keys."""
        call_count = 0

        @cache(ttl=TTL)
        def cached_func(items: frozenset) -> int:
            nonlocal call_count
            call_count += 1
            return call_count

        result1 = cached_func(frozenset({1, 2, 3}))
        result2 = cached_func(frozenset({3, 2, 1}))
...
class TestMixedMutableArguments:
    """Tests for caching behavior with multiple mutable arguments."""

    @pytest.fixture(autouse=True)
    def clear_cache(self):
        MemoryStorage.clear()

    def test_multiple_mutable_args(self):
        call_count = 0

        @cache(ttl=TTL)
        def cached_func(data: dict, items: list, unique: set) -> int:
            nonlocal call_count
            call_count += 1
            return call_count

        result1 = cached_func({"a": 1}, [1, 2], {3, 4})
        result2 = cached_func({"a": 1}, [1, 2], {3, 4})

        assert result1 == result2
        assert call_count == 1

    def test_kwargs_with_mutable_types(self):
        call_count = 0

        @cache(ttl=TTL)
        def cached_func(data: dict | None = None, items: list | None = None) -> int:
            nonlocal call_count
            call_count += 1
            return call_count

        result1 = cached_func(data={"a": 1}, items=[1, 2])
        result2 = cached_func(items=[1, 2], data={"a": 1})
    ...
    def test_positional_vs_keyword_args(self):
        ...
        @cache(ttl=TTL)
        def cached_func(data: dict, items: list) -> int:
            nonlocal call_count
            call_count += 1
            return call_count

        result1 = cached_func({"a": 1}, [1, 2])
        result2 = cached_func(data={"a": 1}, items=[1, 2])

        # Same values passed as positional vs keyword should hit same cache
        assert result1 == result2
        assert call_count == 1

    def test_dict_containing_list_same_order(self):
        ...
        result1 = cached_func(complex1)
        result2 = cached_func(complex2)

        assert result1 == result2
        assert call_count == 1

    def test_dict_different_key_order_produces_different_cache(self):
        """
    ...
    def test_list_containing_dicts(self):
        ...
        result1 = cached_func(list1)
        result2 = cached_func(list2)
...
class TestMutationAfterCaching:
    """Tests to ensure mutations after caching don't affect cache behavior."""

    @pytest.fixture(autouse=True)
    def clear_cache(self):
        MemoryStorage.clear()

    def test_mutating_dict_after_cache_does_not_affect_cache(self):
        call_count = 0
        ...
        original = {"a": 1}
        result1 = cached_func(original)

        # Mutate the dict after caching
        original["b"] = 2

        # Call with original value (now mutated)
        result2 = cached_func({"a": 1})

        # Should still hit cache with {"a": 1}
        assert result1 == result2
        assert call_count == 1

    def test_mutating_list_after_cache_does_not_affect_cache(self):
        call_count = 0
...
class TestUnpicklableArguments:
    ...
    def test_function_argument_raises_type_error(self):
        @cache(ttl=TTL)
        def cached_func(func: Callable) -> int:
            return 42

        with pytest.raises(CacheKeyError):
            cached_func(lambda: None)

    def test_open_file_argument_raises_type_error(self):
        @cache(ttl=TTL)
        def cached_func(sock: socket.socket) -> int:
            return 42
    ...
    def test_custom_object_argument(self):
        call_count = 0

        @cache(ttl=TTL)
        def cached_func(obj: TestUnpicklableArguments.ReducePickleableObject) -> int:
            nonlocal call_count
            call_count += 1
            return call_count

        result1 = cached_func(self.ReducePickleableObject(10))
        result2 = cached_func(self.ReducePickleableObject(10))
